#!/usr/bin/env python
"""
Module for interacting with the land registry
"""
...
class LandRegistryEntry(object):
    """
    Used to store land registry entries
    """
    transaction_id = ""
    price_paid = ""
    transaction_date = ""
...
def get_property_type_name(property_type):
    ...
    if property_type == 'S':
        property_type_name = "Semi-detached"
...
def get_additional_prices_paid(street_name, town):
    ...
    """
    url_template = "http://landregistry.data.gov.uk/app/ppd/ppd_data.csv\
?et%5B%5D=lrcommon%3Afreehold&et%5B%5D=lrcommon%3Aleasehold&limit=all\
&min_date=1+June+2000&nb%5B%5D=true&nb%5B%5D=false\
&ptype%5B%5D=lrcommon%3Adetached&ptype%5B%5D=lrcommon%3Asemi-detached\
&ptype%5B%5D=lrcommon%3Aterraced&street={street}\
...
&tc%5B%5D=ppd%3AadditionalPricePaidTransaction&town={town}"
    ...
    for entry in csv_data.splitlines():
        ...
        item_url = parts[15]
        lr_entry = LandRegistryEntry(transaction_id, item_url)
